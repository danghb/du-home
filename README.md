...
服务启动时预热天气快照，之后默认每 5 分钟在后台刷新。天气使用独立的 `/api/v1/weather` 接口，所有页面共享同一份前端内存缓存。网页首次启动会并行读取各页面数据，之后按当前页面距离动态调度：当前页采用正常周期、左右相邻页使用 2 倍周期、相距两页使用 3 倍周期，并按环形页面的最短距离计算。切页只读取已有缓存，并根据上次成功更新时间补足剩余刷新时间，不会重复建立定时器。网页请求只读取服务端内存快照；HA 暂时不可用时继续保留最后一次成功数据。可通过 `HA_WEATHER_REFRESH_MINUTES` 调整服务端天气刷新周期。
